...
const [repo, branch, workflowName, jobName, artifactPath, outputPath] = process
  .argv.slice(2);
...
const getPipeline = async (index = 0) =>
  (await get(
    `https://circleci.com/api/v2/project/gh/${repo}/pipeline?branch=${branch}`,
  )).items.filter(x => x.errors.length === 0)[index];
...
(async () => {
  ...
  while (!workflow) {
    ...
    workflow = workflows.items.find(x =>
      x.name === workflowName && x.status !== "running"
    );
  ...
  const artifacts = await get(
    `https://circleci.com/api/v2/project/gh/${repo}/${job.job_number}/artifacts`,
  );
  ...
  await writeFile(
    outputPath,
    Buffer.from(await (await fetch(artifact.url)).arrayBuffer()),
  );
})();
