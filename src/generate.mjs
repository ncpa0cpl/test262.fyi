...
for (const file of readdirSync("results2")) {
  ...
  for (let i = 0; i < chunkCount; i++) {
    if (existsSync(join(base, `results${i}.json`))) {
      try {
        const readResults = JSON.parse(
          readFileSync(join(base, `results${i}.json`), "utf8"),
        );
    ...
    if (existsSync(join(base, `time${i}.txt`))) {
      const readTime = parseInt(
        readFileSync(join(base, `time${i}.txt`), "utf8").trim(),
      );
  ...
  if (validResults !== chunkCount) {
    console.log(
      `full results of ${file} is not done yet; expected ${chunkCount} chunks, found ${validResults}`,
    );
  ...
  if (existsSync(join(base, "jsvu.json"))) {
    ...
    versions[file] = jsvu[
      Object.keys(jsvu).find(x => x !== "os" && x !== "engines")
    ]?.trim?.();
  }
...
if (existsSync("results/github-pages/data/engines.json")) {
  const oldEngines = Object.keys(
    JSON.parse(readFileSync("results/github-pages/data/engines.json", "utf8")),
  );
  ...
  const walk = (x) => {
    ...
    for (const k of x.keys()) {
      ...
      if (Array.isArray(y)) {
        ...
        for (const test of y) {
          ...
          for (const engine of engines) {
            ...
            if (out.files[niceFile].engines[engine] === undefined) {
              out.files[niceFile].engines[engine] = 0;
            }

            // const pass = results[engine].find(z => z.file === test.file && z.scenario === test.scenario).result.pass;
            const pass = fileResults[engine][test.file].every(z =>
              z.result.pass
            );
      ...
      for (const engine in walkOut.engines) {
        out.engines[engine] = (out.engines[engine] ?? 0)
          + walkOut.engines[engine];
      }
...
(async () => {
  const rawFeatures = await (await fetch(
    `https://raw.githubusercontent.com/tc39/test262/${test262Rev}/features.txt`,
  )).text();
  ...
  const featureByEdition = new Map(
    Object.entries({
  ...
  let info = {};
  for (
    const line of rawFeatures.split("process-document")[1].split(
      "## Standard",
    )[0].split("\n").filter(x => x)
  ) {
    if (line.startsWith("# https://github.com")) {
      ...
      let spec = await (await fetch(
        `https://raw.githubusercontent.com/tc39/${repo}/HEAD/spec.html`,
      )).text();
      if (spec === "404: Not Found" || spec.includes("stage: -1")) {
        spec = await (await fetch(
          `https://raw.githubusercontent.com/tc39/${repo}/HEAD/spec.emu`,
        )).text();
      }
      if (spec === "404: Not Found" || spec.includes("stage: -1")) {
        spec = await (await fetch(
          `https://raw.githubusercontent.com/tc39/${repo}/HEAD/README.md`,
        )).text();
      }
      ...
      const repoInfo =
        await (await fetch(`https://api.github.com/repos/tc39/${repo}`, {
  ...
  for (const feature of features) {
    ...
    if (edition === undefined) {
      console.warn(`feature '${feature}' has no associated edition`);
    }
    ...
    for (const test of refTests) {
      ...
      if (
        test.attrs && test.attrs.features
        && test.attrs.features.includes(feature)
      ) {
        tests.push(test);
        r.total++;

        if (!editionResults[edition]) {
          editionResults[edition] = { total: 0, engines: {} };
        }
    ...
    for (const engine of engines) {
      ...
      if (editionResults[edition].engines[engine] === undefined) {
        editionResults[edition].engines[engine] = 0;
      }
  ...
  writeFileSync(
    ...
    JSON.stringify(Object.fromEntries(featureResults)),
  );
